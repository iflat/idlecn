...
# EditorWindow.open_module, others, belong here.
    ...
    def loadfile(self, filename):
        try:
            ...
            tkMessageBox.showerror("I/O 错误", str(msg), parent=self.text)
            return False
        chars, converted = self._decode(two_lines, bytes)
        if chars is None:
            tkMessageBox.showerror("解码失败",
                                   "文件 %s\n未能正确解码" % filename,
                                   parent=self.text)
            return False
    ...
    def _decode(self, two_lines, bytes):
        ...
        try:
            ...
            tkMessageBox.showerror(
                title="加载文件失败",
                message="当前安装的Python不能正确识别文件编码 '%s' "\
                "文件将不能正确显示" % name,
                parent = self.text)
            enc = None
        except UnicodeDecodeError:
    ...
    def maybesave(self):
        if self.get_saved():
            return "yes"
        message = "关闭前，是否保存当前文件 %s ?" % (
            self.filename or "未命名")
    ...
    def writefile(self, filename):
        ...
        try:
            ...
            tkMessageBox.showerror("I/O 错误", str(msg),
                                   parent=self.text)
            return False
    ...
    def print_window(self, event):
        ...
        if printPlatform:  #we can try to print for this platform
            ...
            if output:
                output = "打印命令: %s\n" % repr(command) + output
                tkMessageBox.showerror("打印状态", output, parent=self.text)
        else:  #no printing for this platform
            message = "不支持在当前平台下打印: %s" % platform
            tkMessageBox.showinfo("打印状态", message, parent=self.text)
        if tempfilename:
            os.unlink(tempfilename)
        return "break"
