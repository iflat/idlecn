...
class Query(Toplevel):
    ...
    def create_widgets(self):  # Call from override, if any.
        ...
        self.button_ok = Button(
                frame, text='确定', default='active', command=self.ok)
...
class SectionName(Query):
    ...
    def entry_ok(self):
        ...
        if not name:
            self.showerror('没有指定名称.')
            return None
        elif len(name)>30:
            self.showerror('名称长度超过30字符.')
            return None
        elif name in self.used_names:
            self.showerror('名称已存在.')
            return None
        return name
...
class HelpSource(Query):
    ...
    def create_widgets(self):
        ...
        pathlabel = Label(frame, anchor='w', justify='left',
                          text='帮助文件路径: 输入文件 URL 或浏览文件')
        self.pathvar = StringVar(self, self.filepath)
        self.path = Entry(frame, textvariable=self.pathvar, width=40)
        browse = Button(frame, text='浏览', width=8,
    ...
    def path_ok(self):
        ...
        if not path: #no path specified
            self.showerror('没有指定文件或路径.', self.path_error)
            return None
        elif not path.startswith(('www.', 'http')):
            if path[:5] == 'file:':
                path = path[5:]
            if not os.path.exists(path):
                self.showerror('指定的路径不存在.',
                               self.path_error)
                return None
